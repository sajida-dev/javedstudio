            ...
            <div className={`fixed inset-0 z-50 bg-gray-100 bg-opacity-10 ${menuOpen ? "block" : "hidden"}`} onClick={() => setMenuOpen(false)}>
                <nav className="fixed top-0 left-0 bottom-0 w-5/6 max-w-sm py-6 px-6 bg-white border-r overflow-y-auto">
                    ...
                    <div className="mt-auto pt-6">
                        <Link href="/book-now" className="block px-4 py-3 mb-2 text-xs text-center text-white font-semibold bg-blue-600 hover:bg-blue-700 rounded-xl">
                            Book Now
                        </Link>
                    </div>

                    <p className="my-4 text-xs text-center text-gray-400">© {new Date().getFullYear()} Company Name</p>
                </nav>
            </div>
        </>
    );
